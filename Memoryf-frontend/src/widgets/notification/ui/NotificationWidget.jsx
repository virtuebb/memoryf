...
import { getFeedUpfileUrl, getProfileImageUrl } from "../../../shared/api";
...
function NotificationWidget() {
	...
	const markReadLocally = (notificationNo) => {
		setNotifications((prev) =>
			prev.map((n) => (n.notificationNo === notificationNo ? { ...n, isRead: "Y" } : n))
		);
	};
	...
	const renderDeleteSheet = () => {
		...
		return (
			<div className="delete-sheet-overlay" onClick={closeDeleteSheet}>
				<div className="delete-sheet" onClick={(e) => e.stopPropagation()}>
					<button className="delete-sheet-btn danger" onClick={confirmDeleteNotification}>
						삭제
					</button>
					<button className="delete-sheet-btn" onClick={closeDeleteSheet}>
						취소
					</button>
				</div>
			</div>
	...
	return (
		<div className="notification-container">
				...
				{notifications.length === 0 ? (
					...
					notifications.map((noti) => (
						<div
							...
							className={`notification-item ${noti.isRead === "N" ? "unread" : ""}`}
						>
							<div className="noti-content">
								<img
									src={
										noti.senderProfile && noti.senderStatus !== "Y"
											? getProfileImageUrl(noti.senderProfile)
											: defaultProfileImg
									}
									alt="profile"
								...
								<div className="noti-text">
									...
									{noti.type === "GUESTBOOK" && (
										<>
											님이 회원님의{" "}
											<button type="button" className="noti-link" onClick={() => handleGoHome(noti)}>
												홈
											</button>
											에 방명록을 남겼습니다.
										</>
								...
								{noti.feedImage && (
									(() => {
										...
										return isVideo ? (
											<video
												src={`${getFeedUpfileUrl(noti.feedImage)}#t=1.0`}
											...
											<img
												src={getFeedUpfileUrl(noti.feedImage)}
												alt="feed"
												className="noti-feed-img"
												onClick={(e) => {
													e.stopPropagation();
							...
							{noti.type === "FOLLOW_REQUEST" && (
								<div className="noti-actions">
									<button className="btn-accept" onClick={() => handleAccept(noti)}>
										수락
									</button>
									<button className="btn-reject" onClick={() => handleReject(noti)}>
										거절
									</button>
								</div>
							)}

							<div className="noti-more" onClick={() => openDeleteSheet(noti)}>
								⋯
							</div>
						</div>
					))
			...
			{createPortal(renderDeleteSheet(), document.body)}
		</div>
	);
}
