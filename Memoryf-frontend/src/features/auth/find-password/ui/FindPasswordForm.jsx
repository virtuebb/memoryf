...
const FindPasswordForm = () => {
  ...
  const findPassword = async (e) => {
    ...
    try {
      ...
      // ApiResponse: { success, message, data: { exists } }
      if (result?.success && result.data?.exists) {
        // 아이디 저장 -> 비밀번호 변경 시 사용
        sessionStorage.setItem("resetMemberId", userId);
        navigate("/reset-password");
      } else {
        setFindMessage(result?.message || "아이디 또는 이메일이 일치하지 않습니다.");
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <form className="findpw-form" onSubmit={findPassword}>
      ...
      <p className="findpw-desc">아이디와 가입 시 등록한 이메일을 입력하세요.</p>
      ...
      <button type="button" className="findpw-back" onClick={() => navigate(-1)}>
        뒤로가기
      </button>
    </form>
  );
